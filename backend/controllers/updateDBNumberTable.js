    ...
    getCryptoIDsShortnames() {
        ...
        db.connection.query(sql, function(error, results) {
            if (error) {
                ...
                _this.getHistoricalData(results);
            }
        });
    }
    ...
    getHistoricalData(result) {
        result.forEach((data, zoo) => {
            var interval_id = setInterval(() => {
                ...
                let cryptoList = [];
                const request = axios
                    .get("https://min-api.cryptocompare.com/data/histohour?fsym=" + data.crypto_shortname + "&tsym=USD&limit=9&?" + api_key + "")
                    .then(response => {
                ...
                request.then(historical_data => {
                    this.updateDataInTable(historical_data);
                });
            }, 3000);
        });
    }

    updateDataInTable(historical_data) {
        let cryptoList = [];
        if (Array.isArray(historical_data)) {
            data.forEach(bar => {
                // Calculate the Data_id
                //  a) From the time data returned from the api, calculate the hours since current time for each data timepoint
                // b) plug those hours into the DATA_ID_MAP to return the data id
                let data_id = 0;
