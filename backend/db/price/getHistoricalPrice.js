...
function getHistoricalPrice(crypto_info, limit) {
  ...
  return new Promise((resolve, reject) => {
    let request_url = `https://min-api.cryptocompare.com/data/histohour?fsym=${crypto_info.crypto_shortname}&tsym=USD&limit=${limit < 1 ? 1 : limit}&?${api_key}`;
      ...
      const request = get(request_url)
        .then(response => {
          ...
          if (response.data.Response === "Error") {
            ...
            if (
              Array.isArray(historical_data) &&
              Object.entries(historical_data).length !== 0
            ) {
              ...
              if (
                new Date(last_timestamp * 1000).toLocaleDateString() <
                "7/7/2018" &&
                limit === 2000
              ) {
                console.log(
