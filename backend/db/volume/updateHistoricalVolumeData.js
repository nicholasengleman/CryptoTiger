...
const getHistoricalVolume = require("./getHistoricalVolume");
    ...
    getCryptoListTable((err, CRYPTO_LIST_TABLE) => {
        ...
        function healthCheck(i) {
            db.query(sql, [cryptoList[i].crypto_id], function(error, results) {
                ...
                console.log(
                    `${
                        cryptoList[i].crypto_shortname
                    }: updated ${data_age.toFixed(2)} hours ago`
                );
                ...
                if (Math.floor(data_age) > 1) {
                    getHistoricalVolume(
                        ...
                        (err, cryptoData) => {
                            ...
                            db.query(sql, [cryptoData], function(
                                error,
                                results
                            ) {
                                if (error) throw error;
